...
<body>
	<%
		...
		// 빠른 열거는 오른쪽의 데이터 집합이 null일 때는 NullPointerException이 발생
		// 데이터가 1개도 없으면 이 경우는 항상 정상 처리 됩니다.
		if(cookies != null){
			for(Cookie cookie : cookies){
				out.println("<h3>" + cookie.getName() + ":" + cookie.getValue() + "</h3>");
			}
			// 기존에 존재하는 쿠키의 값을 새로 설정해서 추가
			Cookie cookie = new Cookie("id", "choch92");
			response.addCookie(cookie);
			
			// 유효시간을 0으로 설정해서 삭제하기
			cookie = new Cookie("nickname", "choch92");
			cookie.setMaxAge(0);
			response.addCookie(cookie);
